...
build:
	...
	printf "\033[32mDone!\033[0m\n"
	jlink --modulepath /usr/lib/jvm/java-9-openjdk-amd64/jmods:$(OUTPUTF)  --addmods java.base --output $(BUILD)

test:
	@printf "\033[35mShowing Dependencies...\033[0m\n"
	jdeps -modulepath $(OUTPUTF) -s -m simple.program
	./$(BUILD)/bin/java -verbose:classes -verbose:gc -modulepath $(OUTPUTF) -m simple.program/com.opensource.software.$(PROGNAME)
